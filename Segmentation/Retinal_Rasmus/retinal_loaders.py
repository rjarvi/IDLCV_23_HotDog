...
class PATCH_DRIVEDataLoader(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        self.is_train_loader = not load_test
        
        self.crop_size = crop_size
        
        
        assert split in ["Train", "Validation", "Test"], "Invalid set name. Use 'Train', 'Validation', or 'Test'."
        
        if not load_test and split in ["Train", "Validation","Test"]:
            # Training data
            image_folder = os.path.join(data_path, 'training', 'images')
            vessel_mask_folder = os.path.join(data_path, 'training', '1st_manual')  # Vessel masks for training
            fov_mask_folder = os.path.join(data_path, 'training', 'mask')  # Field of view masks for training
        ...
        if split=="Train" and len(self.image_paths) != len(self.vessel_mask_paths):
            raise ValueError("Mismatch between the number of images and vessel masks in the training set.")
        ...
        if split == 'Train':
            ...
            self.fov_mask_paths = self.fov_mask_paths[:train_split]
            self.vessel_mask_paths = self.vessel_mask_paths[:train_split]
        elif split == 'Validation':
            self.image_paths = self.image_paths[train_split:val_split]
            self.fov_mask_paths = self.fov_mask_paths[train_split:val_split]
            self.vessel_mask_paths = self.vessel_mask_paths[train_split:val_split]
        elif split == 'Test':
            self.image_paths = self.image_paths[val_split:]
            self.fov_mask_paths = self.fov_mask_paths[val_split:]
        else:
    ...
    def transform(self, image, vessel_mask, fov_mask):
        ...
        out_image = None
        out_fov_mask = None
        out_vessel_mask = None
        if vessel_mask != None:
            out_image,  out_fov_mask, out_vessel_mask = self.transforms(image, fov_mask,vessel_mask)
        else:
            out_image,  out_fov_mask = self.transforms(image, fov_mask)

        
        toTensor = TF.Compose([TF.ToImage(), TF.ToDtype(torch.float32, scale=True)])
        ...
        if vessel_mask != None:
            out_vessel_mask = toTensor(out_vessel_mask)
        
        out_fov_mask = toTensor(out_fov_mask)
        out_image = toTensor(out_image)
        
        
        
        return out_image, out_vessel_mask, out_fov_mask
    ...
    def __getitem__(self, idx):
        ...
        if vessel_mask is not None:
         ...
         # Create the output dictionary, excluding vessel_mask if it's None
        dictionary = {
            "image": image,
            "fov_mask": fov_mask
        }
        if vessel_mask is not None:
            dictionary["vessel_mask"] = vessel_mask

        return dictionary
    
    
    
# Test split sizes of PATCH_DRIVEDataLoader

train_dataset = PATCH_DRIVEDataLoader(split="Train", crop_size=(128,128))
val_dataset = PATCH_DRIVEDataLoader(split="Validation", crop_size=(128,128))
test_dataset = PATCH_DRIVEDataLoader(split="Test", crop_size=(128,128))

assert len(train_dataset) == 14, "Incorrect number of training samples"
assert len(val_dataset) == 3, "Incorrect number of validation samples"
assert len(test_dataset) == 3, "Incorrect number of test samples"
